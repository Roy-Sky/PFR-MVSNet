...
class PointMVSNet(nn.Module):
    def __init__(self,
        ...
        self.feature_conv_xy = nn.Conv1d(114, 35, 1, bias=False)
        self.feature_conv_yz = nn.Conv1d(114, 35, 1, bias=False)
        self.feature_conv_xz = nn.Conv1d(114, 35, 1, bias=False)
        
        self.structure_feature = Structure_Information(35)


        self.flow_trans = nn.ModuleList()
        self.flow_trans.append(
            PointMVS_Transformer(35)
            )
        
        self.flow_mlp = nn.Sequential(
            SharedMLP(105, flow_channels[:-1]),
    ...
    def forward(self, data_batch, img_scales, inter_scales, isFlow, isTest=False):
        ...
        depth_interval = cam_params_list[:, 0, 1, 3, 1] #10.6
        ...
        if isFlow:
            ...
            def point_flow(estimated_depth_map, interval, image_scale, it):
                feature_collection_xy = []
                feature_collection_yz = []
                feature_collection_zx = []
                ...
                for i in interval_list:
                    ...
                    interval_feature_collection_xy = []
                    interval_feature_collection_yz = []
                    interval_feature_collection_zx = []

                    cam_points = (uv * interval_depth_map.view(batch_size, 1, 1, -1))
                    world_points = torch.matmul(R_inv[:, 0:1, :, :], cam_points - t[:, 0:1, :, :]).transpose(1, 2) \
                        .contiguous().view(batch_size, 3, -1)  
                    ...
                    for conv in chosen_conv:
                        ...
                        interval_feature_collection_xy.append(point_features)
                        interval_feature_collection_yz.append(point_features)
                        interval_feature_collection_zx.append(point_features)
                    
                    xyz = world_points
    
                    interval_feature_collection_xy.append(xyz[:, 0:2, :]) 
                    interval_feature_collection_yz.append(xyz[:, 1:3, :])
                    interval_feature_collection_zx.append(xyz[:, [0, 2], :])

                    
                    interval_feature_xy = torch.cat(interval_feature_collection_xy, dim=1)
                    interval_feature_yz = torch.cat(interval_feature_collection_yz, dim=1)
                    interval_feature_zx = torch.cat(interval_feature_collection_zx, dim=1)

                    feature_collection_xy.append(interval_feature_xy)
                    feature_collection_yz.append(interval_feature_yz)
                    feature_collection_zx.append(interval_feature_zx)

                    xyz_collection.append(xyz)
                   
                feature_xy = torch.stack(feature_collection_xy, dim=2) 
                feature_yz = torch.stack(feature_collection_yz, dim=2) 
                feature_zx = torch.stack(feature_collection_zx, dim=2) 
                ...
                if isTest:
                    def cal_sub_flow(xyz, feature_xy, feature_yz, feature_zx, sub_flow_height, sub_flow_width):
                        nn_idx = get_knn_3d(xyz, len(interval_list), knn=self.k)

                        feature_xy = feature_xy.contiguous().view(batch_size, -1, len(interval_list) * sub_flow_height * sub_flow_width) 
                        feature_xy = self.feature_conv_xy(feature_xy) 
                        feature_yz = feature_yz.contiguous().view(batch_size, -1, len(interval_list) * sub_flow_height * sub_flow_width)
                        feature_yz = self.feature_conv_yz(feature_yz) 
                        feature_zx = feature_zx.contiguous().view(batch_size, -1, len(interval_list) * sub_flow_height * sub_flow_width)
                        feature_zx = self.feature_conv_xz(feature_zx) 
                        feature_xyz = feature_xy + feature_yz + feature_zx
                        structure = self.structure_feature(feature_xyz, nn_idx)
                        aggregation_feature = feature_xyz + structure

                        transformer_feature = []
                        for PointMVS_Transformer in self.flow_trans:
                            transformer_feature = PointMVS_Transformer(aggregation_feature, nn_idx)


                        flow = self.flow_mlp(transformer_feature)
                        ...
                        flow_length = flow_length.contiguous().view(1, -1, 1, 1) * interval.view(-1, 1, 1, 1) # (1, 5, 1, 1)
                    ...
                    if img_scale in [0.125]:
                        flow, flow_prob = cal_sub_flow(xyz, feature_xy, feature_yz, feature_zx, flow_height, flow_width)
                        ...
                        sub_flow_height = flow_height // ratio # 120
                        sub_flow_width = flow_width // ratio # 160

                        feature_xy = feature_xy.view(batch_size, -1, len(interval_list), sub_flow_height, ratio,
                                                sub_flow_width, ratio) 
                        feature_yz = feature_yz.view(batch_size, -1, len(interval_list), sub_flow_height, ratio,
                                                sub_flow_width, ratio) 
                        feature_zx = feature_zx.view(batch_size, -1, len(interval_list), sub_flow_height, ratio,
                                                sub_flow_width, ratio) 
                        ...
                        for i in range(ratio):
                            ...
                            for j in range(ratio):
                                feature_ij_xy = feature_xy[:, :, :, :, i, :, j] 
                                feature_ij_yz = feature_yz[:, :, :, :, i, :, j] 
                                feature_ij_zx = feature_zx[:, :, :, :, i, :, j] 
                                
                                xyz_ij = xyz[:, :, :, :, i, :, j] # (1, 3, 5, 120, 160)
    
                                flow_ij, flow_prob_ij = cal_sub_flow(xyz_ij, feature_ij_xy, feature_ij_yz, feature_ij_zx, sub_flow_height, sub_flow_width)
                    ...
                    nn_idx = get_knn_3d(xyz, len(interval_list), knn=self.k) 
                    feature_xy = feature_xy.contiguous().view(batch_size, -1, len(interval_list) * flow_height * flow_width)
                    feature_xy = self.feature_conv_xy(feature_xy)
                    feature_yz = feature_yz.contiguous().view(batch_size, -1, len(interval_list) * flow_height * flow_width)
                    feature_yz = self.feature_conv_yz(feature_yz)
                    feature_zx = feature_zx.contiguous().view(batch_size, -1, len(interval_list) * flow_height * flow_width)
                    feature_zx = self.feature_conv_xz(feature_zx) 
                    feature_xyz = feature_xy + feature_yz + feature_zx
                    ...
                    flow_length = torch.tensor(interval_list).float().to(img_list.device)
                    flow_length = flow_length.contiguous().view(1, -1, 1, 1) * interval.view(-1, 1, 1, 1) # (b, 5, 1, 1)
