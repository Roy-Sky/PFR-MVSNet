...
def eval_file_logger(data_batch, preds, ref_img_path, folder):
    ...
    for i, k in enumerate(preds.keys()):
        if "flow" in k:
            if "prob" in k:
                out_flow_prob_map = preds[k][0].cpu().permute(1, 2, 0).numpy() # (128,240,5)(256,480,5)(512,960,5)
                
                #ensure prob does not have nan values
                isNanMask = np.isnan(out_flow_prob_map)
                out_flow_prob_map[isNanMask] = 0.0
                
                num_interval = out_flow_prob_map.shape[-1] # 5,5,5
                assert num_interval == interval_list.size
                pred_interval = np.sum(out_flow_prob_map * interval_list, axis=-1) + 2.0 # (128,240)(256,480)(512,960)
                pred_floor = np.floor(pred_interval).astype(np.int)[..., np.newaxis]# (128,240,1)(256,480,1)(512,960,1)
                pred_ceil = pred_floor + 1 # (128,240,1)(256,480,1)(512,960,1)
                pred_ceil = np.clip(pred_ceil, 0, num_interval - 1) # (128,240,1)(256,480,1)(512,960,1)
