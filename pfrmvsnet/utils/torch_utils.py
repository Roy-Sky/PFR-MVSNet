...
def get_knn_3d(xyz, kernel_size=5, knn=20):
    """ Use 3D Conv to compute neighbour distance and find k nearest neighbour
      ...
      Returns:
        idx: (B, D*H*W, k)
    """
    batch_size, _, depth, height, width = list(xyz.size()) # (B, 3, 5, H, W)
    assert (kernel_size % 2 == 1)
    hk = (kernel_size // 2) # 2
    k2 = kernel_size ** 2 # 25
    k3 = kernel_size ** 3 # 125

    t = np.zeros((kernel_size, kernel_size, kernel_size, 1, kernel_size ** 3)) # (5, 5, 5, 1, 125)
    ind = 0
    for i in range(kernel_size):
        for j in range(kernel_size):
            for k in range(kernel_size):
                ...
                ind += 1
    weight = np.zeros((kernel_size, kernel_size, kernel_size, 3, 3 * k3)) # (5, 5, 5, 3, 375)
    weight[:, :, :, 0:1, :k3] = t
    weight[:, :, :, 1:2, k3:2 * k3] = t
    weight[:, :, :, 2:3, 2 * k3:3 * k3] = t
    weight = torch.tensor(weight).float()

    weights_torch = torch.Tensor(weight.permute((4, 3, 0, 1, 2))).to(xyz.device) # (375, 3, 5, 5, 5)
    dist = F.conv3d(xyz, weights_torch, padding=hk) # (B, 375, 5, H, W)

    dist_flat = dist.contiguous().view(batch_size, 3, k3, -1) # (B, 3, 125, -1)
    dist2 = torch.sum(dist_flat ** 2, dim=1) # (B, 125, -1)
